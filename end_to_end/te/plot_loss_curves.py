...
def parse_loss_data(file_path):
    ...
    completed step: <int>, seconds: <float>, TFLOP/s/device: <float>, Tokens/s/device: <float>, total_weights: <int>, loss: <float>
    Returns a list of tuples with the extracted values.
    """
    pattern = re.compile(
        r"completed step: (\d+), seconds: ([\d.]+), TFLOP/s/device: ([\d.]+), Tokens/s/device: ([\d.]+), total_weights: (\d+), loss: ([\d.]+)"
    )
    results = []
...
def main(args):
    ...
    for (dp, tp, tpsp, fsdp), files in configs.items():
        ...
        for log_file in files:
            ...
            plt.plot(steps, losses, marker='', linestyle='-', label=os.path.basename(log_file))
        plt.legend()
        plt.xlabel('Step')
        plt.ylabel('Loss')
